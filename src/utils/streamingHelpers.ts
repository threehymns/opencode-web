...
export const getStreamingState = (
	...
	const isComplete =
		(info.role === "assistant" && "time" in info && !!info.time.completed) ||
		sessionIdle;
...
export const hasDisplayableContent = (
	_info: Message,
	parts: Part[],
): boolean => {
	return parts.some((part) => part.type === "text" || part.type === "tool");
};
