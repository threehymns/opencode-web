  ...
  return (
    <div className="flex items-start gap-3 max-w-full">
      ...
      <div className="flex-1 min-w-0">
        <div className={`rounded-lg p-3 max-w-full space-y-3 ${
...
export const ChatContainer = ({ isLoading, isLoadingSession = false }: ChatContainerProps) => {
  const { messages } = useMessageStoreV2()
      ...
      <div className="space-y-4">
        {isLoadingSession ? (
          ...
          messages.map((messageWithParts) => (
            <MessageRenderer key={messageWithParts.info.id} messageWithParts={messageWithParts} />
          ))
        )}
      </div>
    </ScrollArea>
