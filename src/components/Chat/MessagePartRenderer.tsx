...
import { CheckCircleIcon, XCircleIcon, ClockIcon, PlayIcon, Loader2Icon, ChevronDownIcon, ChevronRightIcon, CircleIcon } from 'lucide-react'
...
const TodoRenderer: React.FC<{ state: ToolState }> = memo(({ state }) => {
  ...
  const parsedTodos = useMemo(() => {
    ...
    } else if (output && typeof output === 'object' && Array.isArray((output as any).todos)) {
        ...
        return (
          <div className="flex items-center gap-2 text-primary">
            ...
            <div className="flex-1">
              <div className="text-sm font-medium">{(state as any).title || 'Creating todo list'}</div>
            </div>
          </div>
        )
            ...
            {parsedTodos.length > 0 ? (
              <div className="space-y-2">
                {parsedTodos.map((todo: TodoItem) => (
                  <div key={todo.id} className="flex items-start gap-3 p-3 bg-muted/50 rounded-lg border">
        ...
        return (
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-destructive">
              ...
              <span className="text-sm font-medium">Failed to create todo list</span>
            </div>
            <div className="bg-destructive/10 rounded p-3 border-l-4 border-destructive">
              <pre className="text-xs whitespace-pre-wrap font-mono text-destructive">{(state as any).error}</pre>
            </div>
          </div>
        )

      default:
        return <div className="text-sm text-muted-foreground">Unknown state</div>
    }
  }
      ...
      case 'running':
        return (
          <div className="flex items-center gap-2 text-primary">
            <Loader2Icon className="h-4 w-4 animate-spin" />
            <div className="flex-1">
              <div className="text-sm font-medium">{(state as any).title || 'Running tool'}</div>
              {(state as any).metadata && (
                <div className="text-xs text-muted-foreground mt-1">
                  {Object.entries((state as any).metadata).map(([key, value]) => (
      ...
      case 'completed':
        return (
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-primary">
              <CheckCircleIcon className="h-4 w-4" />
              <span className="text-sm font-medium">{(state as any).title}</span>
              {((state as any).output || ((state as any).attachments && (state as any).attachments.length > 0)) && (
                <button
              ...
              <div className="space-y-2">
                {(state as any).output && (
                  <div className="bg-muted rounded p-3 border-l-4 border-primary">
                    <pre className="text-xs whitespace-pre-wrap font-mono">{(state as any).output}</pre>
                  </div>
                )}
                {(state as any).attachments && (state as any).attachments.length > 0 && (
                  <div className="space-y-1">
                    <div className="text-xs text-muted-foreground">Attachments:</div>
                    {(state as any).attachments.map((attachment: any, index: number) => (
                      <div key={index} className="text-xs bg-accent p-2 rounded">
...
const MessagePartRendererComponent: React.FC<MessagePartRendererProps> = ({ part }) => {
  const { theme } = useTheme()

  const syntaxStyle = useMemo(() => theme === 'dark' ? oneDark : oneLight, [theme])
    ...
    case 'reasoning':
      return (
        <div className="border-l-4 border-primary pl-4 py-2 bg-accent rounded">
          <div className="text-sm text-accent-foreground font-medium mb-1">Reasoning</div>
          <div className="text-sm text-accent-foreground/80 whitespace-pre-wrap">{(part as any).text}</div>
        </div>
      )
      ...
      return (
        <div className="border rounded p-3 bg-muted">
          <div className="flex items-center gap-2">
            <div className="text-sm font-medium">📎 {(part as any).filename || 'File'}</div>
            <div className="text-xs text-muted-foreground">({(part as any).mime})</div>
          </div>
          {(part as any).source && (
    ...
    case 'step-finish':
      return (
        <div className="flex items-center gap-2 text-primary border-l-4 border-primary pl-4 py-1">
          <CheckCircleIcon className="h-4 w-4" />
          <span className="text-sm">Step completed: {(part as any).reason}</span>
        </div>
      )
    ...
    case 'agent':
      return (
        <div className="flex items-center gap-2 text-secondary border-l-4 border-secondary pl-4 py-1">
          <div className="text-sm font-medium">🤖 Agent: {(part as any).name}</div>
        </div>
      )
    ...
    case 'retry':
      return (
        <div className="flex items-center gap-2 text-accent border-l-4 border-accent pl-4 py-1">
          <div className="text-sm">🔄 Retry attempt {(part as any).attempt}</div>
        </div>
      )
...
const areEqual = (prevProps: MessagePartRendererProps, nextProps: MessagePartRendererProps) => {
  const prev = prevProps.part
  const next = nextProps.part
  if (prev.id !== next.id || prev.type !== next.type) return false
  if (prev.type === 'text' && (prev as any).text !== (next as any).text) return false
  if (prev.type === 'tool' && JSON.stringify((prev as any).state) !== JSON.stringify((next as any).state)) return false
