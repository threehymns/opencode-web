...
export function ProjectSelector({ open, onOpenChange, onSelect }: ProjectSelectorProps) {
  const { projects, fetchProjects, isLoadingProjects, error } = useProjectStore()
  ...
  return (
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      ...
      <CommandList>
        ...
        <CommandEmpty>
          {isLoadingProjects ? 'Loading projects...' : error ? 'Failed to load projects' : 'No projects found.'}
        </CommandEmpty>
            ...
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <span className="text-sm text-muted-foreground">
                  Created: {new Date(project.time.created).toLocaleDateString()}
                </span>
                </div>
              </CommandItem>
            ))}
